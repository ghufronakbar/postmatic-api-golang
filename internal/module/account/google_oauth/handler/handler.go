// internal/module/account/google_oauth/handler/handler.go
...
type Handler struct {
	authSvc *google_oauth_service.GoogleOAuthService
	cfg     *config.Config
}

func NewHandler(authSvc *google_oauth_service.GoogleOAuthService, cfg *config.Config) *Handler {
...
func (h *Handler) LoginGoogleCallback(w http.ResponseWriter, r *http.Request) {
	req := google_oauth_service.GoogleOAuthCallbackInput{
	...
	sessionInput := google_oauth_service.SessionInput{DeviceInfo: clientInfo}
	...
	auth_handler.SetAuthCookies(w, r, h.cfg, res.AccessToken, res.RefreshToken)
