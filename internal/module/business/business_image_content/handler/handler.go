// internal/module/business/business_image_content/handler/handler.go
package business_image_content_handler
...
type Handler struct {
	busInSvc   *business_image_content_service.BusinessImageContentService
	middleware *internal_middleware.OwnedBusiness
}

func NewHandler(busInSvc *business_image_content_service.BusinessImageContentService, ownedMw *internal_middleware.OwnedBusiness) *Handler {
...
func (h *Handler) GetBusinessImageContentsByBusinessRootID(w http.ResponseWriter, r *http.Request) {
	...
	filterQuery := business_image_content_service.GetBusinessImageContentsByBusinessRootIDFilter{
...
func (h *Handler) CreateBusinessImageContent(w http.ResponseWriter, r *http.Request) {
	business, _ := internal_middleware.OwnedBusinessFromContext(r.Context())

	var req business_image_content_service.CreateUpdateBusinessImageContentInput
...
func (h *Handler) UpdateBusinessImageContent(w http.ResponseWriter, r *http.Request) {
	business, _ := internal_middleware.OwnedBusinessFromContext(r.Context())
...
func (h *Handler) DeleteBusinessImageContent(w http.ResponseWriter, r *http.Request) {
	id := chi.URLParam(r, "businessImageContentId")
	intId, err := strconv.ParseInt(id, 10, 64)
	if err != nil {
		response.Error(w, r, errs.NewValidationFailed(map[string]string{"businessImageContentId": "must be int64"}), nil)
